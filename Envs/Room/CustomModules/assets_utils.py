...
def get_3dfront_scene_path(scene_name, scene_data_type="scenes"):
    """
    ...
    threedfront_dataset_path = os.path.join( threedfront_dataset_path, scene_data_type)
    assert scene_name in os.listdir(
...
def download_ig_dataset():
    ...
    url = "https://storage.googleapis.com/gibsonchallenge/gibson_challenge_data_2021.tar.gz"
    file_name = url.split('/')[-1]
    os.system(
        'wget -c --retry-connrefused --tries=5 --timeout=5 {} -O /tmp/{}'.format(url, file_name))
    os.system(
