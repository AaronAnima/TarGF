...
from igibson.scenes.floor_map_utils import process_floor_map
...
class InteractiveIndoorScene(StaticIndoorScene):
    ...
    def __init__(self,
                 ...
                 scene_source="IG",
                 scene_data_type="scenes",
                 floor_map_resolution=0.01,
                 category_to_resize_ratio={},
        ...
        if scene_source == "IG":
            ...
            scene_dir = get_3dfront_scene_path(scene_id, scene_data_type)
        ...
        self.category_to_resize_ratio_by_category = category_to_resize_ratio
        ...
        for link in self.scene_tree.findall('link'):
            if 'category' in link.attrib:
                ...
                resize_ratio = 1
                if category in category_to_resize_ratio:
                    resize_ratio = category_to_resize_ratio[category]
        ...
        self.pack_wall_objs(floor_map_resolution)
    ...
    def add_object(self,
        ...
        try:
            ...
            print("May have encountered bad object.")
            with open('/home/user/anaconda3/envs/pb/lib/python3.6/site-packages/igibson/log.txt', 'a') as f:
                f.write(self.scene_dir + '\n')
        ...
        if joint_type != "floating":
            ...
            ET.SubElement(new_joint, "child",
                          dict([("link", object_name)]))
            ET.SubElement(new_joint, "axis",
                          dict([("xyz", "0.0 0.0 1.0")]))
            ET.SubElement(added_object.object_tree.getroot(), "link",
                          dict([("name", joint_parent)]))  # "world")]))
    ...
    def floor_map_to_world(self, xy):
        axis = 0 if len(xy.shape) == 1 else 1
        return np.flip((xy - self.floor_map_size / 2.0) * self.floor_map_resolution, axis=axis)

    def world_to_floor_map(self, xy):
        return np.flip((xy / self.floor_map_resolution + self.floor_map_size / 2.0)).astype(np.int)

    def get_wall_info(self, floor_map_resolution):
        layout_dir = os.path.join(self.scene_dir, "layout")
        floor_map_path = os.path.join(layout_dir, 'floor_map.png')
        floor_map = Image.open(floor_map_path)

        height, width = floor_map.size
        assert height == width, 'room floor map is not a square'
        self.floor_map_default_resolution = 0.01
        self.floor_map_resolution = floor_map_resolution
        self.floor_map_size = int(height *
                                self.floor_map_default_resolution /
                                self.floor_map_resolution)
        floor_map = np.array(floor_map.resize(
            (self.floor_map_size, self.floor_map_size), Image.NEAREST))

        corners, _, edges, normals = process_floor_map(floor_map)
        return corners, (edges, normals)

    def pack_wall_objs(self, floor_map_resolution):
        wall_info = self.get_wall_info(floor_map_resolution)
        self.wall_obj_dicts = []
        self.wall_obj_lists = []
        corners, walls = wall_info
        corners = self.floor_map_to_world(corners)
        for edge, normal in zip(*walls):
            idx_from, idx_to = edge
            edge_from, edge_to = corners[idx_from], corners[idx_to]
            self.wall_obj_dicts.append({"from": edge_from, "to": edge_to, "normal": normal})
            self.wall_obj_lists.append(np.hstack([edge_from, edge_to, normal]))
